...
class SGOptimizer(object):
    def __init__(self, func_obj, var_list, eta, name):
        ...
        for i, v in enumerate(self.var_list):
            v_hist = tf.get_variable(name = self._name + '_hist_' + str(i), initializer = tf.zeros(tf.shape(v)), dtype=tf.float32)
            self.var_hist_list.append(v_hist)
        gradients = tf.gradients(self.func_obj, self.var_list)
        self.var_update_list=[]
        for g, v, v_hist in zip(gradients, self.var_list, self.var_hist_list):
            ...
            self.var_update_list.append(v1)

    def maximize(self):
        return self.var_update_list
